...
use mock::{
	ACAJointSwap, AUSDBTCPair, AUSDDOTPair, AUSDJointSwap, DOTBTCPair, DexModule, Event,
	ExtBuilder, ListingOrigin, Origin, Runtime, System, Tokens, ACA, ALICE, AUSD,
...
fn abort_provisioning_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(DexModule::initial_share_exchange_rates(AUSDDOTPair::get()), Default::default());
		assert_eq!(
			DexModule::trading_pair_statuses(AUSDBTCPair::get()),
			TradingPairStatus::<_, _>::Provisioning(ProvisioningParameters {
				min_contribution: (1_000_000_000_000u128, 1_000_000_000_000u128),
